  ...
  return (
    ...
    >
      <button className="flex items-center text-foreground/80 hover:text-primary transition-colors font-semibold text-[15px] outline-none py-2 cursor-pointer">
        {item.name}
        <ChevronDown
          className={`ml-1.5 w-4 h-4 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute top-full left-1/2 transform -translate-x-1/2 pt-6 z-[110] ${item.dropdown.length > 1 ? 'w-[420px]' : 'w-[220px]'}`}
          >
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed w-full z-[100] transition-all duration-300 ${scrolled || mobileMenuOpen ? "glass py-3" : "bg-transparent py-5"}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <Link
              to="/"
              className="relative flex items-center cursor-pointer z-[110] transform hover:scale-105 transition-transform"
            >
              <img
              ...
              <div className="px-4 py-4 space-y-4 flex flex-col">
                {navItems.map((item, idx) =>
                  item.dropdown ? (
                    ...
                    >
                      <span className="text-foreground/50 font-bold uppercase text-xs mb-3 block">
                        {item.name}
                      </span>
                        ...
                        {item.dropdown
                          ...
                          .map((link, lidx) => (
                            <Link
                              key={lidx}
                              to={link.href}
                              className="block text-foreground/80 hover:text-primary font-medium text-[15px]"
                            >
                              {link.name}
                            </Link>
                          ))}
                    ...
                    <a
                      key={idx}
                      href={item.href}
                      className="block text-foreground/80 hover:text-primary transition-colors font-medium py-2 border-b border-white/5 last:border-0 text-[15px]"
                    >
                      {item.name}
                    </a>
                  ) : (
